...
impl Contract {
    ...
    pub fn slash_stake(
        ...
        match self.users.get_mut(&penalized_user) {
            Some(user) => {
                require!(
                    user.locked_stake >= penalty,
                    "User does not have enough locked stake.",
                );

                Promise::new(benefiary).transfer(penalty);

                user.locked_stake = user.locked_stake.saturating_sub(penalty);
                user.total_stake = user.total_stake.saturating_sub(penalty);

                if user.total_stake.is_zero() {
                    self.users.remove(&penalized_user);
                } else {
                    user.available_stake = user.total_stake.saturating_sub(user.locked_stake);
                }
            }
            ...
            None => {
                log!("User {} does not exist", penalized_user);
            }
        }
    }
}
